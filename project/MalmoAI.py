...
class MalmoAI():
	...
	def handleStenchBreeze(self, stench, breeze):
		...
		if (not stench and not breeze):
			...
			if (stench and breeze and not self.wumpusFound):
				print("here")
				if (north[1] < self.northMax):
					if (self.wumpusMap[north[0]][north[1]] != 0 and self.pitMap[north[0]][north[1]] != 0):
				...
				if (east[0] < self.eastMax):
					if (self.wumpusMap[east[0]][east[1]] != 0 and self.pitMap[east[0]][east[1]] != 0):	
				...
				if (south[1] >= 0):
					if (self.wumpusMap[south[0]][south[1]] != 0 and self.pitMap[south[0]][south[1]] != 0):
				...
				if (west[0] >= 0):
					if (self.wumpusMap[west[0]][west[1]] != 0 and self.pitMap[west[0]][west[1]] != 0):
	...
	def getNextAction(self):
		...
		coordsToMove = self.dijkstra((picked))
	...
	def getOut(self):
		self.movementQueue = self.convertCoordToAction(self.dijkstra((0,0)))

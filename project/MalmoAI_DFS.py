...
class MalmoAI():
	...
	def getMovement(self, stench, breeze, glitter, bump):
		...
		if (self.haveGold and not self.gettingOut):
	...
	def doActions(self):
		...
		if (direction == "NORTH"):
			print(direction)
			self.previousAction = direction
			self.trace.insert(0, (self.current[0], self.current[1] + 1))
			self.current = (self.current[0], self.current[1] + 1)
		...
		if (direction == "EAST"):
			print(direction)
			self.previousAction = direction
			self.trace.insert(0, (self.current[0] + 1, self.current[1]))
			self.current = (self.current[0] + 1, self.current[1])
	...
	def getNextAction(self):
		...
		coordsToMove = self.tracesteps(picked)
		...
		print("coord to get there: ", coordsToMove)
		print("actions to get there: ", list(actions.queue))

		return actions

	def tracesteps(self, dest):
		north, east, south, west = self.getAdjacents(dest)
		coordsToMove = None
		for i in range(len(self.trace)):
			if (self.trace[i] == north or self.trace[i] == east or self.trace[i] == south or self.trace[i] == west):
				coordsToMove = list(self.trace[:i+1])
				coordsToMove.append(dest)
				print("coords to translate ", coordsToMove)
				break
		return coordsToMove
	...
	def getOut(self):
		print("LEAVING")
		self.movementQueue = self.convertCoordToAction(self.tracesteps((0,0)))
		self.movementQueue.put("CLIMB")
		print("actions to get out: ", list(self.movementQueue.queue))

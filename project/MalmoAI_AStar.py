...
class MalmoAI():
	...
	def getMovement(self, stench, breeze, glitter, bump):
		...
		self.movementQueue = self.getNextAction(1)
	...
	def getNextAction(self, radius):
		...
		for x in list(self.safeSpots):
			...
			if (x[0] >= self.eastMax or x[1] >= self.northMax):
				...
				continue
			current = len(self.dijkstra(self.current, x)) + self.surrounding_heuristic(x, radius)
			print(current)
			if (current < smallest):
				picked = x
				smallest = current
